...
class ExamController extends Controller
{
	...
	public function store(ExamRequest $request, Exam $exam, Student $student, Question $question)
	{
		...
		$assignedQuestions = $exam->assignQuestions(
			$request->input('course_type'),
			$request->input('no_questions'),
			$students
		);

		if ($assignedQuestions['status'] && $request->has('completed_date')) {
		...
		if ($request->has('completed_date') && !$assignedQuestions['status']) {
	...
	public function update(ExamRequest $request, Exam $exam, Student $student, Question $question)
	{
		...
		if (
			count($oldStudentsIds) && ($exam->certifications->first()->course_type != $request->input('course_type')) ||
	...
	{
		return [
			'pass_percentage', 'title', 'zoom_link', 'no_questions', 'duration', 'start_date', 'display', 'type', 'proctor', 'location', 'stack', 'domain', 'instructor_id', 'country_id', 'address', 'class_title', 'course_type',
			'course_start_date',
			'course_end_date',
			'class_id',
			'has_certification'
