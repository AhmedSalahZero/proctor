...
class Student extends Authenticatable
{
	...
	protected $fillable = [
		...
		'company', 'month_of_birth', 'day_of_birth', 'country', 'city', 'postal_code', 'position', 'image', 'employee_id', 'image','last_name'
	];
	...
	// {
	// 	return $this->hasOne(Certification::class , 'student_id','ID')->orderBy('id','desc');
	// }
	// public function 
	...
	public function isExamSubmitted(User_exam $userExam)
	{
		return $userExam->Finished_At;
	}
	...
	public function getCertificationTopics()
	{
		...
		switch ($userPercentage) {
			case $userPercentage >= 80 && $userPercentage < 90:
				return [
					'Kick Detection:' => ['Pit Gain' => 'Explain why it is important to monitor pit levels at all times when the rig is connected to the well (for example, open hole always has a potential to flow; tested barriers may fail).'], 'Post Shutin Monitoring & Activities:' => ['Analysis of Shut-In Conditions' => 'Explain Shut-In Drill pipe Pressure (SI DPP) and Shut-In Casing Pressure (SICP).'],
			...
			case $userPercentage >= 90 && $userPercentage <= 100:
				...
				return [
					'Equipment:' => ['Manifolds, Piping, and Valves' => 'Explain the function of this equipment in the well control process.'], 'Post Shutin Monitoring & Activities:' => ['Analysis of Shut-In Conditions' => 'Explain Shut-In Drill pipe Pressure (SI DPP) and Shut-In Casing Pressure (SICP).'],
					'Risk Management:' => [' Managing Change during a Well Kill' => 'Identify problems that can happen during a well control operation (for example, plugging; washouts; losses; equipment issues).'],
			...
			case $userPercentage < 75:
				...
				return [
					'Abnormal Pressure Warning Signs:' => ['Trend Analysis' => 'Give examples of how abnormal pressure affects primary control in the wellbore.'], 'Causes of Kicks' => ['Barrier Failure' => 'Give examples of how a kick can be caused by the failure of a mechanical barrier (for example, a barrier that is retaining pressure can fail and allow formation fluids to flow up the hole; when displacing the well to a lighter weight fluid, a barrier can fail).'],
	...
	public function getUpcomingClasses() // upcoming courses 
	{
		...
		$this->instructorExams->load('students.certifications')->each(function ($exam) use (&$filterExams) {
			
			(new Exam)->getClassType(true, $exam, $filterExams);
			
		});
		// dd($filterExams['past'][0]);
		return $filterExams;
	...
	public function getFrontendNameFormatted()
	{
