...
class Exam extends Model
{
	...
	function getRemainingTime($userExamId): string
	{
		...
		if ($this->type == self::FLEXIBLETYPE) {
			...
			if ($userExam->first_entered_at) {
				$firstEnteredAt  = $userExam->first_entered_at;
				$endData = Carbon::make(now())->diff(Carbon::make($firstEnteredAt)->addMinutes($this->duration));
	...
	{
		return Carbon::make($this->start_date)->format(getDefaultDateTimeFormat(false));
	...
	{
		if ($this->display && $this->started_on) {
			return Carbon::make($this->started_on)->format(getDefaultDateTimeFormat(false));
		}
		if ($this->display) {
			return $this->getStartDateFormatted();
		}
		return 'n/a';
	}
	...
	{
		if ($this->ended_on) {
			return Carbon::make($this->ended_on)->format(getDefaultDateTimeFormat(false));
		}
		if ($this->end && $this->end_date) {
			return Carbon::make($this->end_date)->format(getDefaultDateTimeFormat(false));
		}
		return 'n/a';
	}
	...
	public function getCourseDate()
	{
		...
		if ($this->getCourseEndDateFormatted()) {
			return $this->getCourseEndDateFormatted();
		}
		return '-';
	}
