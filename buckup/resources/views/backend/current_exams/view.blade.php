...
{{--                </form>--}}
                ...
                <table class="table table-striped- table-bordered table-hover table-checkable" id="table22">
                    ...
                    <tbody>
                          ...
                          {{-- @foreach($user_exams as $key=>$user_exam)
                          @if(\App\Models\UserAnswer::where('Test_ID',$user_exam->ID)->first())
                              <tr >

                                  <td >{{++$key}}</td>
                                  <td>
                                      @if($user_exam->student)
                                      {{$user_exam->student->User_Name}}
                                  @endif
                                  </td>

                                  <td>{{$user_exam->entered_at ? \Carbon\Carbon::make($user_exam->entered_at)->format('M d , Y g:i A') : ''}}</td>
                                  ...
                                  <td  >
                                        {{ \App\Models\UserAnswer::where('Test_ID',$user_exam->ID)->whereNotNull('Answer')->count() }}
                                  </td>
                                  <td>
                              
                                      @if($user_exam->student && $user_exam->exam && $user_exam->student->certifications()->where('exam_id',$user_exam->exam->id)->first() && $user_exam->student->certifications()->where('exam_id',$user_exam->exam->id)->first()->course)
                                          {{$user_exam->student->certifications()->where('exam_id',$user_exam->exam->id)->first()->course->name}}
                                  ...
                                  <td  hidden>
                                      ...
                                      @if($user_exam->student && $user_exam->exam && count($user_exam->exam->students) && ($user_exam->exam->students->where('ID',$user_exam->student->ID)->first()))
                                      <a data-student="{{ $user_exam->student->ID }}"
                                         data-exam="{{ $user_exam->exam->id }}"
                                         data-status="{{ (int) $user_exam->exam->students->where('ID',$user_exam->student->ID)->first()->HasAbsenceExam() }}"
                                         id="absence_{{ $user_exam->student->ID }}_{{ $user_exam->exam->id }}"
                                         title="Absence"
                                         href="{{ route('student.toggle.absence', [$user_exam->student->ID, $user_exam->exam->id]) }}"
                                         class="btn btn-sm btn-clean btn-icon btn-icon-md absence_class"
                                         type="button">

                                          @if ($user_exam->exam->students->where('ID',$user_exam->student->ID)->first()->HasAbsenceExam())
                                              <i data-student="{{ $user_exam->student->ID }}"
                                                 data-exam="{{ $user_exam->exam->id }}"
                                                 data-status="{{ (int) $user_exam->exam->students->where('ID',$user_exam->student->ID)->first()->HasAbsenceExam()  }}"
                                                 id="on_id"
    ...
    <script type="text/javascript">
        document.addEventListener('click', function(event) {
         ...
         $('#table22').dataTable({
                ...
                , "pageLength": 10
                , "columns": [
                    {render:function(a,b,row){return row['order'];} },
                     {render:function(a,b,row){return row['studentName'];} },
                     {render:function(a,b,row){return row['entered_at'];} },
                     {render:function(a,b,row){return row['no_questions'];} },
                     {render:function(a,b,row){return row['no_answers'];} },
                     {render:function(a,b,row){return row['subject'];} }
